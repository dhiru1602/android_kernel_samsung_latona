...
static void svnet_read_wq(struct work_struct *work)
{
	...
	if (tmp_itor) {
		d = t - tmp_itor;
		dev_dbg(&sn->ndev->dev, "int_to_read %llu ns\n", d);
		tmp_itor = 0;
		if (time_max_itor < d)
			time_max_itor = d;
	}
	...
	dev_dbg(&sn->ndev->dev, "read_time %llu ns\n", d);
	if (d > time_max_read)
		time_max_read = d;
}
...
static void svnet_write_wq(struct work_struct *work)
{
	...
	if (tmp_xtow) {
		d = t - tmp_xtow;
		dev_dbg(&sn->ndev->dev, "xmit_to_write %llu ns\n", d);
		tmp_xtow = 0;
		if (d > time_max_xtow)
			time_max_xtow = d;
	}
	...
	stat.st_wq_state = 4;
	d = cpu_clock(smp_processor_id()) - t;
	dev_dbg(&sn->ndev->dev, "write_time %llu ns\n", d);
	if (d > time_max_write)
		time_max_write = d;
}
